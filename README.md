...
Метрики нужны для оценки качества двух основных элементов нашей RAG системы, а именно, качества отбора новостей из базы данных по запросу и качества фильтрации оценочных суждений. Для оценки качества отбора новостей, используется $\textbf{relevance\_ratio}$ - отношение количества новостей после фильтрации и изначального количества подобранных новостей по запросу. Заметим, что низкие значения этой метрики могут быть связаны и с тем, что запрос очень специфический и на него не нашлось подходящих новостей. Однако, при достаточно общем запросе, эта метрика оценивает качество отбора новостей. Для оценки качества фильтрации оценочных суждений используется предобученая модель https://huggingface.co/GroNLP/mdebertav3-subjectivity-multilingual - модель классификации теста на два класса: субъективный или объективный. Под объективностью новости будем понимать вероятность того, что новость объективна, согласно предсказанию модели. Мы используем несколько метрик, для оценки качества фильтрации оценочных суждений: $\textbf{avg\_orig\_objectivity}$ - средняя объективность отобранных новостей до фильтрации, $\textbf{avg\_filtered\_objectivity}$ - средняя объективность фильтрованных новостей. $\textbf{avg\_irrelevant\_objectivity}$ - средняя объективность отброшенных новостей, $\textbf{ratio\_subj\_to\_obj}$ - доля изначально субъективных новостей, которые были преобразованны в объективные и $\textbf{ratio\_obj\_to\_subj}$ - доля изначально объективных новостей, которые были преобразованны в субъективные.